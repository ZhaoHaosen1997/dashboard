...
### 备份配置（每个系统独立设置，默认关闭）
- [ ] 数据库路径配置（每个系统可单独填写 db 文件路径）
- [ ] 备份开关（默认：关闭，需手动开启）
- [ ] 自动备份间隔（可配置：每日 / 每 7 日 / 每 30 日，默认不自动）
- [ ] 保留备份份数（可配置，默认 3）
- [ ] 配置入口：管理后台 → 每个系统的编辑弹窗内
...
- [ ] 自动触发备份（仅对已开启备份的系统生效）：
  ...
  - **间隔上限**：超过设定间隔未备份则强制触发
  - **启动防抖**：1 小时内已备份则跳过
- [ ] 备份文件命名：`YYYYMMDD_HHMMSS.db`
- [ ] 按保留份数自动清理旧备份

#### 坚果云 WebDAV 备份
- [ ] WebDAV 配置（URL、用户名、密码/token）
- [ ] 手动触发上传 / 下载
- [ ] 展示云端和本地最新备份时间和数据库文件大小，让你决定同步方向
- [ ] 不自动同步，仅手动操作

#### 恢复功能
- [ ] 查看备份列表（按系统选择）
- [ ] 恢复流程：停止服务 → 备份当前为「恢复前版本」→ 覆盖数据库 → 重启服务
- [ ] 「恢复前版本」命名加 `pre_restore_` 前缀
