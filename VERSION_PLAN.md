...
**目标**：CPU / 内存 / 磁盘 / GPU 超过阈值时首页卡片变红、标题闪烁

- [ ] 前端阈值配置（默认：CPU > 90%、内存 > 90%、磁盘 > 90%、GPU > 85%）
- [ ] `loadMetrics()` 中检查实时值，超阈值时卡片加红色边框 + 脉冲动画
- [ ] 持续超阈值 N 次（避免抖动）后触发浏览器桌面通知（`Notification API`）
- [ ] 阈值和开关可通过前端修改，存 localStorage
